...
export const AppShell = ({ children, title, rightSlot, showLogo = false, fillHeight = false }: AppShellProps) => {
 ...
 <div
 className="relative mx-auto flex h-[100dvh] max-w-md flex-col overflow-hidden animate-phone-spin-in [transform-origin:center_center] md:h-[min(92dvh,900px)] md:rounded-[52px] md:border-2 md:border-foreground shadow-card"
 >
 <StatusBar />

 {/* iOS-style header */}
 {(title || showLogo) && (
 <header className="z-30 flex shrink-0 items-center justify-between border-b border-foreground bg-background px-5 pt-[max(env(safe-area-inset-top),2.25rem)] pb-3 ">
