...
export const SwipeDeck = ({ compact = false }: { compact?: boolean }) => {
 ...
 <div className="absolute inset-0 translate-y-3 scale-[0.96] rounded-lg bg-carbon border border-border " aria-hidden>
 <img src={next.image} alt="" className="w-full h-full object-cover rounded-lg " loading="lazy" />
 </div>
 ...
 {/* Verified */}
 <div className="absolute top-3 left-3 flex items-center gap-1 px-2 py-0.5 rounded-sm bg-success border border-success">
 ...
 <p className="text-[10px] text-muted-foreground mt-0.5 font-mono uppercase tracking-wider">{current.location}</p>
 </div>
 </div>
 ...
 <p className="text-sm leading-relaxed text-foreground">{current.building}</p>
 </div>
 )}
 ...
 Tap <span className="text-gold">target</span> to connect · <span className="text-foreground">Ã-</span> to pass
 </p>
 )}
 </div>
 );
...
const DecisionBtn = ({
 ...
 ghost: "border border-border bg-card hover:border-foreground text-muted-foreground hover:text-foreground",
 }[variant];
 return (
 <button
 {...props}
