...
const Waitlist = () => {
 ...
 <Link
 to="/how"
 className="group inline-flex items-center gap-2 rounded-full border border-border bg-card px-6 py-3 text-sm font-medium text-foreground transition-colors hover:border-primary hover:bg-accent"
 >
 See the full flow
 <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
 </Link>
 ...
 {/* FOOTER --------------------------------------------------- */}
 <footer className="mx-auto max-w-6xl px-5 md:px-8 pb-10 pt-6 border-t border-border">
...
const Pillar = ({
 ...
 <div className="h-px w-12 bg-primary mb-5 transition-[width] duration-500 group-hover:w-20" />
 ...
 <ul className="space-y-2 border-l border-primary pl-4 mt-5 opacity-0 transition-opacity duration-500 group-hover:opacity-100 group-hover:delay-200">
 {details.map((d) => (
 <li
 key={d}
...
const RoleCard = ({
 ...
 <article className="group self-start rounded-2xl border border-border bg-card p-7 md:p-9 transition-colors">
 <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-primary mb-4">
 {kind}
 </p>
 ...
 <div className="grid grid-rows-[0fr] transition-[grid-template-rows] duration-500 ease-out group-hover:grid-rows-[1fr]">
 <div className="overflow-hidden">
 <div className="mt-6 pt-5 border-t border-primary opacity-0 transition-opacity duration-500 group-hover:opacity-100 group-hover:delay-200">
...
// Decorative blurred backdrop removed - the site is fully opaque, no
// glow orbs anywhere. Kept the stub so any leftover <BlurredBackdrop />
// reference renders nothing instead of throwing.
